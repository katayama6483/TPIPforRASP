 ...
 * @version 2.00  2020/04/24 katayama 画像伝送 header 変更
 *
...
/** set JTP_Header
 * @fn     int set_JTP_data(jtp_t* snd, u_char flag, u_char seq, u_char frm_no, u_char vga, char* data, u_long size, u_long itv_tm, u_char cam_no, u_long j_size)
 ...
 * @param  frm_no  : frame number
 * @param  vga     : Display resolution [0:QVGA, 1:VGA, 2:SVGA, 3:XGA, 4:HD, 5:SXGA, 6:UXGA, 7:FHD]
 * @param  data    : JPEG block data top address
 * @param  size    : JPEG block data size
 * @param  itv_tm  : Interval time
 * @param  TMstamp : time stamp
 * @param  cam_no  : camera no <0 - 3>
 * @param  j_size  : JPEG size
 * @retval send data size
 *
 */
static int set_JTP_data(jtp_t* snd, u_char flag, u_char seq, u_char frm_no, u_char vga, char* data, u_long size, u_long itv_tm, u_char cam_no, u_long j_size)
{
...
//		size++;
	...
	snd->jtp_h.seq      = seq;
	snd->jtp_h.vga      = vga;
	snd->jtp_h.frm_no   = frm_no;
	snd->jtp_h.size     = size;
	snd->jtp_h.TMstmp   = itv_tm;
	snd->jtp_h.j_size   = j_size;
...
static void  t_jtp( unsigned long arg )
{
	...
	while (t_jtp_Flg > 0) {
		...
		if (t_jtp_Flg == 1) {	// 1st Phase [get JPEG data]
			if (rcv_cnt > 0) {
//				jpeg_data_p_base = (u_char*)&jpeg_data[0];
//				actual_size = JPEG_read(&jpeg_data_p_base, sizeof(jpeg_data), &info);
				actual_size = JPEG_read(&jpeg_data_p_base, 0, &info);
//				printf("JPEG_read()=%d\n", actual_size);
				if (actual_size > 0) {
					not_ready_cnt = 0;
					JPEG_size    = actual_size;
					...
					jpeg_err_cnt = 0;
					itv_tm = info.itv_tm;
//					itv_tm = (u_long)(calc_itv_time(&info.st_ts, &info.ed_ts) * 1000);
/*					if (actual_size == 0) {
...
//					jpeg_data_p = &jpeg_data[MAX_BLOCK_SIZE * block_no];	// set block top position of jpeg data
					jpeg_data_p = jpeg_data_p_base + (MAX_BLOCK_SIZE * block_no);	// set block top position of jpeg data
				...
				if (block_size >= 0) {
					snd_sz  = set_JTP_data(&snd_buf, flag, seq, frm_no, _vga, jpeg_data_p, block_size, itv_tm, (u_char)(cam_no & 0x03), JPEG_size);
...
//				printf("[%s:%s] chang rq_JPEG_cam_no = %X \n", __FILE__, __func__, rq_JPEG_cam_no);
			...
			if (rq_JPEG_vType != JPEG_vType) {
				printf("[%s:%s] chang JPEG type(%d -> %d) \n", __FILE__, __func__, JPEG_vType, rq_JPEG_vType);
				JPEG_close();
...
/** get JTP parameter
 *  @fn     int JTP_get_param(t_jtp_param* param)
 *  @param  param  : JTP param
 *  @retval == 0 : OK
 *  @retval <  0 : error
 *
 */
int JTP_get_param(t_jtp_param* param)
{
	int err = -1;
	
	if (t_jtp_Flg > 0) {
		param->v_type = JPEG_vType;
		param->q_fact = JPEG_kbps;
		param->cam_no = JPEG_cam_no;
		param->v_fps  = 0;
		err = 0;
	}
	return err;
