 ...
 * @version 2.00  2020/04/24 katayama 画像伝送(JTP header 変更)
 *
...
//#define JTCP_VER      (130)
#define JTCP_VER      (200)		// 画像伝送 test version
...
static int check_JTCP_Header(jtcp_t* rcv, int rcv_sz)
{
	...
	if (rcv->jtcp_h.mark == '*') err = 1;
		...
		if (rcv->jtcp_h.ver != JTCP_VER) err = ERR_JTCP_VER;	// check communication vresion (test version)
